...
public class Testcap3Ctrl : MonoBehaviour {
    ...
    {
        if (answer.Equals(ListAnswers[4]))
        {
            Points++;
            audiocontroller.PlayAudio(0);
        }
    ...
    public void fintest()
    {
        ...
        if (finalpoints >= 70)
        {
            PlayerPrefs.SetInt("module1", 3);
            reintentarbtn.SetActive(false);
            textofinal.text = "Felicidades has obtenido la llave del capitulo 3";
